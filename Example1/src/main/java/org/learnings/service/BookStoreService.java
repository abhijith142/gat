...
public class BookStoreService {
    ...
    public void persistBook(Book book) throws ClassNotFoundException, SQLException {
        ...
        PreparedStatement insertPublisher = connection.prepareStatement("INSERT INTO Publisher(id, publisherName) values(?,?)");
        insertPublisher.setString(1, book.getPublisher().getId());
        insertPublisher.setString(2, book.getPublisher().getPublisherName());

        insertPublisher.executeUpdate();
        insertPublisher.close();

        PreparedStatement insertBook = connection.prepareStatement("INSERT INTO Book(isbn, bookName, pub_id) values(?,?,?)");
        insertBook.setString(1, book.getIsbn());
        insertBook.setString(2, book.getBookName());
        insertBook.setString(3, book.getPublisher().getId());
